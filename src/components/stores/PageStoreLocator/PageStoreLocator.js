...
export default {
  ...
  methods: {
    hasChannels() {
      return (
        ...
        this.channels.results.length > 0
      );
    },
    ...
    openingHours(channel) {
      const field = (channel?.customFieldsRaw || []).find(
        ({ name }) => name === "openingTimes"
      );
      const hours = field?.value?.en || "";
      return hours;
    },
  },
  data: () => ({
    ...
    if (this.$store.state?.channel) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          // eslint-disable-next-line
          alert(
            `Error: The Geolocation service failed: ${err}`
          );
        }
      );
    } else {
  ...
  apollo: {
    channels: {
      ...
      variables() {
        return {
          where: `geoLocation within circle(${(this &&
            this.center &&
            this.center.lng) ||
            -78.9052195},
              ${(this && this.center && this.center.lat) ||
                35.9937228},
              ${((this && this.searchRadius) || 1000000) *
                1609.4})
            `,
        };
      },
      result() {
        console.log(
          "have channels?",
          this.channels.results.length
        );
        this.markers =
          this.channels &&
          this.channels.results.map((c) => ({
            position: getLocationFromChannel(c),
          }));
      },
    },
  },
